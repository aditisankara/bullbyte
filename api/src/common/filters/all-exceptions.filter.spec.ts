...
function makeHost(responseMock: { status: jest.Mock; json: jest.Mock; headersSent?: boolean }) {
  return {
    switchToHttp: () => ({
...
describe('AllExceptionsFilter', () => {
  ...
  it('maps HttpException to correct shape', () => {
    ...
    expect(json).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: 404, error: 'NOT_FOUND', code: 'NOT_FOUND', details: {} }),
    );
  });

  it('maps ThrottlerException (429) to RATE_LIMIT_EXCEEDED when code provided', () => {
    const err = new HttpException(
      { statusCode: 429, error: 'TOO_MANY_REQUESTS', code: 'RATE_LIMIT_EXCEEDED', details: {} },
      429,
    );
    filter.catch(err, host);
    expect(status).toHaveBeenCalledWith(429);
    expect(json).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: 429, error: 'TOO_MANY_REQUESTS', code: 'RATE_LIMIT_EXCEEDED' }),
    );
  });
