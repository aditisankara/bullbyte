...
describe('MlSidecarService', () => {
	...
	beforeEach(() => {
		mockHttpService = {
			get: jest
				.fn()
				.mockReturnValue(of({ data: { status: 'ok', service: 'ml' } })),
		};
		mockConfigService = {
			getOrThrow: jest.fn().mockReturnValue('http://ml-sidecar:8000'),
		};
		service = new MlSidecarService(
